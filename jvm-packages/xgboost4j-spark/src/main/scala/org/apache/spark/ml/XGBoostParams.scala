...
trait XGBoostParams extends PredictorParams {
  ...
  final val useExternalCache: BooleanParam = new BooleanParam(this, "use_external_cache",
  ...
  final val nWorkers: Param[Int] = new IntParam(this, "nWorkers",
    "No. of distributed workers to use for training the model.",
    ParamValidators.gtEq(1))
  ...
  setDefault(booster -> "gbtree", silent -> 0, nWorkers -> 2, eta -> 0.3,
    ...
    baseScore -> 0.5, seed -> 0, numClasses -> 2, rounds -> 10, useExternalCache -> true)

  def setUseExternalCache(value: Boolean): this.type = set(useExternalCache, value)

  final def getUseExternalCache(): Boolean = $(useExternalCache)
  ...
  def setNWorkers(value: Int): this.type = set(nWorkers, value)

  final def getNWorkers: Int = $(nWorkers)
